...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        print(i)
        return dict(
            image_paths=self.image_path_list[i], output_texts=self.caption_list[i]
        )

    def collate(self, instances):
        image_paths, output_texts = tuple(
            [instance[key] for instance in instances]
            for key in ("image_paths", "output_texts")
        )
        return dict(image_paths=image_paths, output_texts=output_texts)
...
class SupervisedAudioVisualDataset(Dataset):
    ...
    def __init__(
        ...
        if (
            audio_data_path != ""
            and "audio" in data_type
            and audio_data_path is not None
        ):
        ...
        if (
            image_data_path != ""
            and "image" in data_type
            and image_data_path is not None
        ):
        ...
        if (
            video_data_path != ""
            and "video" in data_type
            and video_data_path is not None
        ):
        ...
        if self.use_whisper:
            ...
            if len(audio) > 30 * self.sample_rate and self.sin_pos:
                ...
                return dict(
                    ...
                    output_texts=copy.deepcopy(self.audio_caption_list[i])
    ...
    def get_image(self, i):
        ...
        return dict(
            ...
            output_texts=copy.deepcopy(self.image_caption_list[i]),
            modality="image",
        )
    ...
    def get_video(self, i, videopath=None):
        ...
        if self.training:
            if self.use_npy:  # npy training
                return dict(
                    image_paths=videopath,
                    output_texts=copy.deepcopy(self.video_caption_list[i]),
                    modality="video",
                )
    ...
    def get_audioimage(self, i):
        ...
        if isinstance(image_data["image_paths"], list):
            ...
            if self.audio_only:
                ...
                audio_data = self.get_audio(i)
                image_data["output_texts"][0]["value"] = (
                    "In the image, " + image_data["output_texts"][0]["value"].lower()
                )
    ...
    def collate(self, instances):
        ...
        for instance in instances:
            ...
            if instance["modality"] == "video":
                ...
                orig_paths.append(
                    instance["orig_paths"] if "orig_paths" in instance else ""
                )
                raw_audios.append(
            ...
            if len(instance["output_texts"][1]["value"].split()) > 500:
                trigger_reduce = max(
                    trigger_reduce,
                    len(instance["output_texts"][1]["value"].split()) // 500,
                )
