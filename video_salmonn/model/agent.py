...
class DeepSpeedAgent:
    def __init__(self, model, args):
        ...
        ds_params["scheduler"]["params"]["total_num_steps"] = (
            self.args["total_steps"] / 2
        )
        ds_params["scheduler"]["params"]["warmup_num_steps"] = max(
            10, int(self.args["total_steps"] * self.args["warmup_rate"]) / 8
        )
        if (
            self.args["world_size"]
            * ds_params["gradient_accumulation_steps"]
            * ds_params["train_micro_batch_size_per_gpu"]
            != ds_params["train_batch_size"]
        ):
