...
from transformers import (BertTokenizer, StoppingCriteria,
                          StoppingCriteriaList, WhisperFeatureExtractor)
...
def build_one_instance(
    tokenizer, conversation, generate=False, prompt=None, use_llama2=False
):
...
def process_batch_instance(
    ...
    for conversation in batch_of_conversations:
        text_list, one_input_ids, one_target_ids, inst = build_one_instance(
            tokenizer,
            conversation,
            generate=generate,
            prompt=prompt,
            use_llama2=use_llama2,
        )
...
class OpenLLAMAPEFTModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        with open(
            "./prompt/alignment_speech_multitask.json", "r", encoding="utf-8"
        ) as f:
        ...
        if not self.pure_aud:
            if self.use_blip:
                ...
                self.visual_encoder = create_eva_vit_g(224, 0, False, "fp16")
                ...
                qformer_path = (
                    "./ckpt/pretrained_ckpt/instruct_blip_vicuna13b_trimmed.pth"
                )
        ...
        if self.use_whisper:
            ...
            self.whispertransform = WhisperFeatureExtractor.from_pretrained(
                whispermodel, cache_dir=self.cache_dir
            )
        ...
        if self.use_beats:
            beatsmodel = (
                "./ckpt/pretrained_ckpt/BEATs_iter3_plus_AS2M_finetuned_on_AS2M_cpt2.pt"
            )
        ...
        if not self.pure_aud:
            if self.use_qformer == "true":
                if self.use_whisper and not self.speech_qformer:
                    self.speech_pre_qformer_proj = nn.Linear(
                        self.speech_encoder.config.d_model, self.visual_hidden_size
                    )
                    if self.use_beats:
                        self.beats_pre_qformer_proj = nn.Linear(
                            self.beats.cfg.encoder_embed_dim, self.visual_hidden_size
                        )
                if self.use_nemo and not self.speech_qformer:
                    self.speech_pre_qformer_proj = nn.Linear(
                        512, self.visual_hidden_size
                    )  # TODO change 512 to a number in config
                if self.early_align and self.alignmode == 2:
                    if self.speech_qformer:
                        self.visual_hidden_size = (
                            self.visual_hidden_size + self.speech_encoder.config.d_model
                        )
                    else:
                        if self.bilinear_pooling:
                            self.bp_proj = nn.Linear(
                                self.visual_hidden_size,
                                self.visual_hidden_size,
                                bias=False,
                            )
                            self.bp_vis = nn.Linear(
                                self.visual_hidden_size,
                                self.visual_hidden_size,
                                bias=False,
                            )
                            self.bp_whisper = nn.Linear(
                                self.visual_hidden_size,
                                self.visual_hidden_size,
                                bias=False,
                            )
                            if self.use_beats:
                                self.bp_beats = nn.Linear(
                                    self.visual_hidden_size,
                                    self.visual_hidden_size,
                                    bias=False,
                                )
                ...
                if self.instructblip_video:
                    ...
                    self.video_Qformer.bert.embeddings.load_state_dict(
                        Qformer_embeddings
                    )
                else:
                ...
                if self.ext_groupsize is not None:
                    self.llama_proj = nn.Linear(
                        self.video_Qformer.config.hidden_size * 3,
                        self.llama_model.config.hidden_size,
                    )
                elif self.low_groupsize is not None or self.high_groupsize is not None:
                    self.llama_proj = nn.Linear(
                        self.video_Qformer.config.hidden_size * 2,
                        self.llama_model.config.hidden_size,
                    )
                else:
                    self.llama_proj = nn.Linear(
                        self.video_Qformer.config.hidden_size,
                        self.llama_model.config.hidden_size,
                    )
                ...
                if self.speech_qformer:
                    ...
                    (
                        self.speech_Qformer,
                        self.speech_query_tokens,
                    ) = self.init_video_Qformer(
                        num_query_token=args.get(
                            "num_speech_query", self.num_video_query_token
                        ),
                    ...
                    if self.instructblip_video:
                        self.speech_Qformer.resize_token_embeddings(
                            len(self.bert_tokenizer)
                        )
                    else:
                ...
                if self.early_align and self.alignmode == 3:
                    ...
                    self.joint_frame_position_embedding = nn.Embedding(
                        4096, self.joint_size
                    )
                    self.ln_joint = nn.LayerNorm(self.joint_size)
                    (
                        self.joint_Qformer,
                        self.joint_query_tokens,
                    ) = self.init_video_Qformer(
                        num_query_token=self.num_video_query_token,
                    ...
                    self.llama_proj_joint = nn.Linear(
                        self.joint_Qformer.config.hidden_size,
                        self.llama_model.config.hidden_size,
                    )
                    ...
                    self.joint_frame_position_embedding = (
                        self.video_frame_position_embedding
                    )
                    if self.ext_groupsize is not None:
                        (
                            self.low_Qformer,
                            self.low_query_tokens,
                        ) = self.init_video_Qformer(
                            num_query_token=int(
                                self.num_video_query_token
                                * self.ext_groupsize[0]
                                / self.groupsize
                            ),
                            vision_width=self.visual_hidden_size,
                            num_hidden_layers=self.seglen if self.xsegalign else 2,
                        ...
                        (
                            self.high_Qformer,
                            self.high_query_tokens,
                        ) = self.init_video_Qformer(
                            num_query_token=int(
                                self.num_video_query_token
                                * self.ext_groupsize[1]
                                / self.groupsize
                            ),
                            vision_width=self.visual_hidden_size,
                            num_hidden_layers=self.seglen if self.xsegalign else 2,
            ...
            if self.speech_qformer:
                ...
                if self.ps_instruct:
                    bert_model = torch.load(
                        "/mnt/bn/audio-visual-llm-data/torch_home/hub/checkpoints/bert-base-uncased/pytorch_model.bin",
                        map_location="cpu",
                    )
                    bert_emb_state_dict = {
                        k: v for k, v in bert_model.items() if "bert.embeddings" in k
                    }
                    self.speech_Qformer.load_state_dict(
                        bert_emb_state_dict, strict=False
                    )
                    ...
                    for (
                        ...
                        self.speech_Qformer.bert.embeddings.word_embeddings.named_parameters()
                    ):
                        param.requires_grad = False
                    for (
                        name,
                        ...
                        self.speech_Qformer.bert.embeddings.position_embeddings.named_parameters()
                    ):
                        param.requires_grad = False
        ...
        if args["delta_ckpt_path"]:
            delta_ckpt = torch.load(
                args["delta_ckpt_path"], map_location=self.llama_model.device
            )
    ...
    def encode_video(
        ...
        if self.use_blip:
            if is_img and self.img_hi_rs:
                inputs, video_masks = data.load_and_transform_vision_data_blip(
                    video_paths,
                    self.device,
                    self.training,
                    hi_rs=True,
                    hi_rs_cfg=self.img_hi_rs_cfg,
                )
            else:
                inputs, video_masks = data.load_and_transform_video_data_blip(
            ...
            with torch.no_grad():
                ...
                if self.instructblip and instruction_inds is not None:
                    ...
                    instruction_mask = (
                        instruction_inds.attention_mask.unsqueeze(1)
                        .repeat(1, nframes, 1)
                        .view(bsize * nframes, -1)
                    )
                    Qformer_atts = torch.cat([query_atts, instruction_mask], dim=1)
                    input_ids = (
                        instruction_inds.input_ids.unsqueeze(1)
                        .repeat(1, nframes, 1)
                        .view(bsize * nframes, -1)
                    )
                ...
                video_embeds = video_embeds.reshape(
                    bsize, nframes * self.num_query_token, video_embeds.size(-1)
                )
                video_masks = (
                    video_masks.unsqueeze(-1)
                    .repeat(1, 1, self.num_query_token)
                    .view(bsize, -1)
                )
                if self.video_window_size < nframes:
                    pad_len = (
                        nframes // self.video_window_size + 1
                    ) * self.video_window_size - nframes
                    pad_len = int(pad_len * self.num_query_token)
                    n_windows = int(nframes // self.video_window_size + 1)
                    pad_video_embeds = video_embeds.new_zeros(
                        bsize, pad_len, video_embeds.size(-1)
                    )
                    pad_video_masks = video_masks.new_zeros(bsize, pad_len)
                    video_embeds = torch.cat(
                        [video_embeds, pad_video_embeds], dim=1
                    ).view(bsize * n_windows, -1, video_embeds.size(-1))
                    video_masks = torch.cat([video_masks, pad_video_masks], dim=1).view(
                        bsize * n_windows, -1
                    )
        else:
            video_features, video_masks = data.load_and_transform_video_data_full(
                video_paths, self.device
            )
        ...
        if self.use_qformer == "true":
            ...
            if self.instructblip_video and instruction_inds is not None:
                query_atts = torch.ones(
                    video_query_tokens.size()[:-1], dtype=torch.long
                ).to(video_embeds.device)
                if self.video_window_size < nframes:
                    instruction_mask = (
                        instruction_inds.attention_mask.unsqueeze(1)
                        .repeat(1, n_windows, 1)
                        .view(bsize * n_windows, -1)
                    )
                    input_ids = (
                        instruction_inds.input_ids.unsqueeze(1)
                        .repeat(1, n_windows, 1)
            ...
            if self.video_window_size < nframes:
                video_embeds = video_embeds.view(
                    bsize, -1, video_embeds.size(-1)
                )  # bsz x Q*n_windows x embsize
                inputs_llama = self.llama_proj(video_embeds)  # bsz x Q x llama_size
                atts_llama = (
                    (video_masks.sum(dim=-1) != 0)
                    .unsqueeze(1)
    ...
    def load_video_npy_train(self, video_paths, instructs):
        ...
        for idx, video_path in enumerate(video_paths):
            ...
            if instruct not in video_npy_dict.item().keys():
                ...
                print(
                    f'error, instruct "{instruct}" not in npy, use another to replace "{new_instruct}"'
                )
                instruct = new_instruct
            video_embed = video_npy_dict.item()[instruct]
            video_embed = (
                torch.from_numpy(video_embed)
                .squeeze(0)
                .to(self.device)
        ...
        for video_embed in video_embed_list:
            if video_embed.size(0) < maxlen:
                ...
                padded_video_masks_list.append(
                    [1] * video_embed.size(0) + [0] * diffsize
                )
                video_embed = torch.cat(
                    [video_embed, video_embed.new_zeros(diffsize, video_embed.size(1))],
                    dim=0,
                )
            else:
        ...
        if self.use_whisper:
            ...
            with torch.no_grad():
                ...
                audio_embeds = self.speech_encoder(
                    audio_paths.to(self.llama_model.dtype).to(
                        self.speech_encoder.device
                    ),
                    return_dict=True,
                ).last_hidden_state
                if self.use_beats:
                    ...
                    for raw_audio in raw_audios:
                        ...
                        beats_feature_lens = torch.tensor(
                            [feature.shape[0] for feature in beats_feature]
                        )
                        beats_feature = pad_sequence(
                            beats_feature, batch_first=True, padding_value=0
                        )
                        # if beats_feature.ndim == 1:
                        #     beats_feature.unsqueeze(0)
                        beats_feature_mask = torch.arange(
                            beats_feature.shape[1]
                        ).unsqueeze(0) >= beats_feature_lens.unsqueeze(1)
                        beats_features.append(
                            self.beats.extract_features(
                                beats_feature.to(self.llama_model.dtype).to(
                                    self.beats.device
                                ),
                                padding_mask=beats_feature_mask.to(self.beats.device),
                                feature_only=True,
                            )[0]
                        )
                    max_feature_len = max(
                        [feature.size(1) for feature in beats_features]
                    )
                    for i in range(len(beats_features)):
                        if beats_features[i].size(1) < max_feature_len:
                            beats_features[i] = F.pad(
                                beats_features[i],
                                (0, 0, 0, max_feature_len - beats_features[i].size(1)),
                                "constant",
                                0,
                            )
            ...
            if not self.speech_qformer and not self.pure_aud:
                ...
                if self.use_beats:
                    if beats_features.size(1) < audio_embeds.size(1):
                        beats_features = F.pad(
                            beats_features,
                            (0, 0, 0, audio_embeds.size(1) - beats_features.size(1)),
                            "constant",
                            0,
            ...
            inputs = {
                ModalityType.AUDIO: data.load_and_transform_audio_data_fulllen(
        ...
        if self.use_qformer == "true":
            ...
            if not self.pure_aud:
                ...
                frame_position_embeddings = (
                    self.video_frame_position_embedding(position_ids).mean() * 0
                )
                frame_hidden_state = (
                    frame_position_embeddings + audio_embeds
                )  # audio/speech do not use pos enc
            else:
                frame_hidden_state = audio_embeds  # audio/speech do not use pos enc
            frame_atts = torch.ones(audio_embeds.size()[:-1], dtype=torch.long).to(
                audio_embeds.device
            )
            # frame_atts = video_masks.long()
            speech_query_tokens = self.speech_query_tokens.expand(
                audio_embeds.shape[0], -1, -1
            )
            if visual_query is not None and not self.pure_aud:
                speech_query_tokens = torch.cat(
                    [speech_query_tokens, visual_query], dim=1
                )
            if (
                self.instructblip_video
                and instruction_inds is not None
                and not self.pure_aud
            ):
                query_atts = torch.ones(
                    speech_query_tokens.size()[:-1], dtype=torch.long
                ).to(audio_embeds.device)
                Qformer_atts = torch.cat(
                    [query_atts, instruction_inds.attention_mask], dim=1
                )
                ...
                seg_pos_embs = (
                    sinusoidal_position(self.n_pos, frame_hidden_state.shape[-1])
                    .to(audio_embeds.device)
                    .to(frame_hidden_state.dtype)
                )
                ...
                for seg, n in zip(seg_hidden_state, num_seg):
                    if self.sin_pos:
                        seg = (
                            (seg + seg_pos_embs[:n].unsqueeze(1).expand(seg.shape))
                            .view(-1, seg.shape[-1])
                            .unsqueeze(0)
                        )
                    else:
                ...
                if self.ps_instruct:
                    ...
                    audio_query_output = self.speech_Qformer.bert(
                        ...
                        query_embeds=speech_query_tokens.to(
                            self.speech_Qformer.bert.device
                        ),
                        encoder_hidden_states=frame_hidden_state.to(
                            self.speech_Qformer.bert.device
                        ),
                        encoder_attention_mask=frame_atts.to(
                            self.speech_Qformer.bert.device
                        ),
                        return_dict=True,
                    )
            ...
            length = torch.tensor(
                [s.size(0) for s in seg_inputs_llama], device=audio_embeds.device
            ).unsqueeze(1)
            col_indices = torch.arange(
                inputs_llama.size(1), device=audio_embeds.device
            ).unsqueeze(0)
    ...
    def encode_image(
        ...
        if self.use_blip:
            inputs = data.load_and_transform_vision_data_blip(
                image_paths, self.device, self.training
            )
    ...
    def sequence_align_v2(
        ...
        if aud_T > vid_T:
            diff_T = aud_T - vid_T
            video_pad = video_embeds.new_zeros(
                bsize, diff_T, video_size, video_embeds.size(-1)
            )
            video_embeds = torch.cat([video_embeds, video_pad], dim=1)
        elif aud_T < vid_T:
            diff_T = vid_T - aud_T
            audio_pad = audio_embeds.new_zeros(
                bsize, diff_T, audio_size, audio_embeds.size(-1)
            )
            audio_embeds = torch.cat([audio_embeds, audio_pad], dim=1)
        audio_token_padding = audio_embeds.new_zeros(
            bsize, audio_embeds.size(1), video_size - audio_size, audio_embeds.size(-1)
        )
        ...
        if self.bilinear_pooling:
            if self.use_beats:
                vis_feat, whisper_feat, beats_feat = torch.split(
                    concat_features, self.visual_hidden_size, dim=-1
                )
                bp_feat = self.bp_proj(
                    F.tanh(self.bp_vis(vis_feat))
                    * F.tanh(self.bp_whisper(whisper_feat))
                    * F.tanh(self.bp_beats(beats_feat))
                )
                concat_features = bp_feat + vis_feat + whisper_feat + beats_feat
            else:
                vis_feat, whisper_feat = torch.split(
                    concat_features, self.visual_hidden_size, dim=-1
                )
                bp_feat = self.bp_proj(
                    F.tanh(self.bp_vis(vis_feat))
        ...
        if self.ext_groupsize is not None:
            ...
            if vid_T % self.ext_groupsize[1] != 0:
                ...
                concat_feature_paddings = concat_features.new_zeros(
                    bsize, diff_T * video_size, concat_features.size(-1)
                )
                concat_features = torch.cat(
                    [concat_features, concat_feature_paddings], dim=1
                )
                total_mask = torch.cat(
                    [total_mask, total_mask.new_zeros(bsize, diff_T * video_size)],
                    dim=1,
                )
                vid_T += diff_T
            high_features = concat_features.view(
                bsize * hgroups,
                self.ext_groupsize[1] * video_size,
                concat_features.size(-1),
            )
            high_mask = total_mask.view(
                bsize * hgroups, self.ext_groupsize[1] * video_size
            )

            lgroups = vid_T // self.ext_groupsize[0]
            low_features = concat_features.view(
                bsize * lgroups,
                self.ext_groupsize[0] * video_size,
                concat_features.size(-1),
            )
            low_mask = total_mask.view(
                bsize * lgroups, self.ext_groupsize[0] * video_size
            )

            low_embeds = self.ln_joint(low_features)  # B x 32*T_max x D
            position_ids = torch.arange(
                low_embeds.size(1), dtype=torch.long, device=low_embeds.device
            )
            position_ids = position_ids.unsqueeze(0).expand(low_embeds.size(0), -1)
            frame_position_embeddings = self.joint_frame_position_embedding(
                position_ids
            )
            ...
            position_ids = torch.arange(
                high_embeds.size(1), dtype=torch.long, device=high_embeds.device
            )
            ...
            high_query_tokens = self.high_query_tokens.expand(
                high_embeds.shape[0], -1, -1
            )
            ...
            high_features = concat_features.view(
                bsize * hgroups,
                self.high_groupsize * video_size,
                concat_features.size(-1),
            )
            high_mask = total_mask.view(
                bsize * hgroups, self.high_groupsize * video_size
            )
        ...
        if self.groupsize >= 1:
            ...
            if self.low_groupsize is not None:
                ...
                low_features = concat_features.view(
                    bsize * lgroups,
                    self.low_groupsize * video_size,
                    concat_features.size(-1),
                )
                low_mask = total_mask.view(
                    bsize * lgroups, self.low_groupsize * video_size
                )
                ...
                low_hidden_state = frame_position_embeddings + low_embeds
                low_query_tokens = self.low_query_tokens.expand(
                    low_embeds.shape[0], -1, -1
                )
            ...
            concat_features = concat_features.view(
                bsize * ngroups, self.groupsize * video_size, concat_features.size(-1)
            )
        ...
        position_ids = torch.arange(
            total_embeds.size(1), dtype=torch.long, device=total_embeds.device
        )
        ...
        joint_query_tokens = self.joint_query_tokens.expand(
            total_embeds.shape[0], -1, -1
        )
        ...
        if self.xsegalign and ngroups > self.seglen:
            total_embeds = total_embeds[:, : self.num_video_query_token].reshape(
                bsize, ngroups, -1, total_embeds.size(-1)
            )
            sel_index = (
                torch.linspace(
                    self.seglen - 1, ngroups - 1, steps=ngroups // self.seglen + 1
                )
                .long()
                .to(self.device)
            )
        ...
        if (
            self.groupsize >= 1
            and self.ext_groupsize is None
            and self.low_groupsize is None
            and self.high_groupsize is None
        ):
            if add_time:
                tlabels = np.round(
                    np.tile((np.arange(vid_T) + 1), bsize) * 0.5 * self.groupsize,
                    decimals=1,
                ).tolist()
                tlabels = [str(t) + " seconds" for t in tlabels]
                tlabel_tokens = self.llama_tokenizer(
                    tlabels, add_special_tokens=False
                ).input_ids
                if self.args["use_lora"] == "true":
                    tlabel_embs = [
                        self.llama_model.model.model.embed_tokens(
                            torch.tensor(t).to(self.llama_model.model.model.device)
                        )
                        for t in tlabel_tokens
                    ]
                else:
                    tlabel_embs = [
                        self.llama_model.model.embed_tokens(
                            torch.tensor(t).to(self.llama_model.model.device)
                        )
                        for t in tlabel_tokens
                    ]
                ...
                for f_emb, t_emb in zip(feat_embs, tlabel_embs):
                    final_embs[n // vid_T].append(
                        torch.cat([f_emb.squeeze(0), t_emb], dim=0)
                    )
                    n += 1
    ...
    def sequence_align(
        ...
        audio_embeds = self.ln_speech(audio_embeds)
        position_ids = torch.arange(
            video_size, dtype=torch.long, device=video_embeds.device
        )
        position_ids = position_ids.unsqueeze(0).expand(video_embeds.size(0), -1)
        frame_position_embeddings = self.video_frame_position_embedding(position_ids)
        frame_hidden_state = frame_position_embeddings + video_embeds
        video_query_tokens = self.video_query_tokens.expand(
            video_embeds.size(0), -1, -1
        )
        audio_query_tokens = self.speech_query_tokens.expand(
        ...
        inputs_llama = inputs_llama.view(
            bsize, -1, inputs_llama.size(-1)
        )  # B x T_max*Q x llama_size
        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(
            self.device
        )  # bsz x 1
    ...
    def prompt_wrap(
        ...
        if audio_embs is not None:
            p_sep = "<SEP>"
            p_sep_tokens = self.llama_tokenizer(
                p_sep, return_tensors="pt", add_special_tokens=False
            ).to(self.device)
        # peft model need deeper call
        if self.args["use_lora"] == "true":
            p_before_embeds = self.llama_model.model.model.embed_tokens(
                p_before_tokens.input_ids.to(self.llama_model.model.model.device)
            ).expand(
                batch_size, -1, -1
            )  # bsz x s1 x embed_dim
            p_after_embeds = self.llama_model.model.model.embed_tokens(
                input_ids.to(self.llama_model.model.model.device)
            ).expand(
                batch_size, -1, -1
            )  # bsz x s2 x embed_dim
            if audio_embs is not None:
                p_sep_embeds = self.llama_model.model.model.embed_tokens(
                    p_sep_tokens.input_ids
                ).expand(
                    batch_size, -1, -1
                )  # bsz x s3 x embed_dim
        else:
            ...
            if audio_embs is not None:
                p_sep_embeds = self.llama_model.model.embed_tokens(
                    p_sep_tokens.input_ids
                ).expand(
                    batch_size, -1, -1
                )  # bsz x s3 x embed_dim
        ...
        if self.early_align and self.alignmode == 3:
            ...
            empty_targets = (
                torch.ones(
                    [batch_size, 1 + p_before_embeds.size(1) + joint_embeds.size(1)],
                    dtype=torch.long,
                )
                .to(self.device)
                .fill_(-100)
            ...
            if audiomasks is not None:
                ...
                joint_masks = torch.cat(
                    [audiomasks, audiomasks.new_ones(batch_size, 1)], dim=-1
                ).to(self.device)
                joint_masks = (
                    joint_masks.unsqueeze(-1)
                    .repeat(1, 1, self.num_video_query_token)
                    .view(batch_size, -1)
                )
            ...
            if audiomasks is not None:
                ...
                visual_masks = (
                    audiomasks[:, 0]
                    .unsqueeze(1)
                    .repeat(1, img_embeds.size(1))
                    .to(self.device)
                )
                audio_masks = (
                    audiomasks[:, 1]
                    .unsqueeze(1)
                    .repeat(1, audio_embs.size(1))
                    .to(self.device)
                )
                atts_prefix = torch.ones(
                    [batch_size, 1 + p_before_embeds.size(1)], dtype=torch.long
                ).to(self.device)
                sep_masks = torch.ones(
                    [batch_size, p_sep_embeds.size(1)], dtype=torch.long
                ).to(self.device)
                atts_prefix = torch.cat(
                    [atts_prefix, visual_masks, sep_masks, audio_masks], dim=1
                )
            else:
                atts_prefix = torch.ones(
                    [
            ...
            empty_targets = (
                torch.ones(
                    [batch_size, 1 + p_before_embeds.size()[1] + img_embeds.size(1)],
                    dtype=torch.long,
                )
                .to(self.device)
                .fill_(-100)
        ...
        attention_mask = torch.cat(
            [atts_prefix, attention_mask.to(atts_prefix.device)], dim=1
        )
    ...
    def calc_diversity_loss(self, query):
        ...
        modulus = torch.einsum(
            "bij,bjk->bik", modulus.unsqueeze(-1), modulus.unsqueeze(1)
        )
        cos_sim = dotprod / (modulus + 1e-9)
        diag_mask = (1 - torch.eye(modulus.size(1), device=self.device)).unsqueeze(0)
        ave_sim = (cos_sim * diag_mask).sum() / (
            (modulus.size(1) ** 2 - modulus.size(1)) * modulus.size(0)
        )
        return ave_sim
    ...
    def forward(self, inputs, reduction=True, generate=False, generate_config=None):
        ...
        if generate and len(output_texts[0]) > 2:
            ...
            for n in range(len(output_texts[0]) // 2):
                tmp_texts = [[output_texts[0][2 * n], output_texts[0][2 * n + 1]]]
                if n == 0:
                ...
                if self.instructblip:
                    ...
                    dummy_instruct = self.bert_tokenizer(
                ...
                if inputs["modality"] == "image":
                    img_embeds, img_query = self.encode_image(
                        image_paths, instruction_ids
                    )
                    if self.use_whisper:
                        dummy, _, _ = self.encode_audio(
                            dummy_audio_path, dummy_instruct, raw_audios=dummy_raw_audio
                        )
                        ...
                        if not self.training:
                            if self.early_align and self.alignmode == 2:
                                ...
                                video_embs = (
                                    img_embeds.unsqueeze(1)
                                    .repeat(1, Tvideo, 1, 1)
                                    .view(img_embeds.size(0), -1, img_embeds.size(2))
                                )
                                video_masks = torch.ones(
                                    video_embs.size()[:-1], dtype=torch.long
                                ).to(video_embs.device)
                                audiomasks = (
                                    torch.tensor([1, 0])
                                    .unsqueeze(0)
                                    .repeat(video_masks.size(0), 1)
                                    .to(video_embs.device)
                                )
                    ...
                    img_embeds, _, video_query = self.encode_video(
                        image_paths, instruction_ids
                    )
                    if self.diversity:
                        diversity_loss += self.calc_diversity_loss(video_query)
                elif inputs["modality"] == "audio":
                    img_embeds, audio_query, atts_llama = self.encode_audio(
                        image_paths,
                        instruction_ids,
                        raw_audios=inputs["raw_audios"],
                        earlyalign=self.early_align,
                    )
                    ...
                    if self.early_align:  # TODO fix wrong dim
                        ...
                        if self.use_beats:
                            video_embs = img_embeds.new_zeros(
                                img_embeds.size(0), Tvideo, img_embeds.size(2) // 2
                            )
                        else:
                            video_embs = img_embeds.new_zeros(
                                img_embeds.size(0), Tvideo, img_embeds.size(2)
                            )
                        video_masks = torch.ones(
                            video_embs.size()[:-1], dtype=torch.long
                        ).to(video_embs.device)
                        ...
                        img_embeds, _, audio_query = self.sequence_align_v2(
                            video_embs, video_masks, img_embeds, audiomasks
                        )
                elif inputs["modality"] == "audioimage":
                    image_paths = list(zip(*image_paths))
                    query_mask = 1
                    if self.cascaded == "audiogrounding":
                        audio_embeds, audio_query, _ = self.encode_audio(
                            image_paths[0], instruction_ids, earlyalign=self.early_align
                        )
                        ...
                        img_embeds, img_query = self.encode_image(
                            ...
                            instruction_ids,
                            earlyalign=self.early_align,
                            audio_query=audio_query * query_mask,
                        )
                        ...
                        audio_embeds, audio_query, _ = self.encode_audio(
                            ...
                            instruction_ids,
                            earlyalign=self.early_align,
                            visual_query=img_query * query_mask,
                        )
                    elif self.cascaded == "bothgrounding":
                        _, pre_audio_query, _ = self.encode_audio(
                            image_paths[0], instruction_ids, earlyalign=self.early_align
                        )
                        _, pre_img_query = self.encode_image(
                            image_paths[1], instruction_ids, earlyalign=self.early_align
                        )
                        if inputs["audiomasks"] is not None:
                            query_mask = (
                        ...
                        audio_embeds, audio_query, _ = self.encode_audio(
                            ...
                            instruction_ids,
                            earlyalign=self.early_align,
                            visual_query=pre_img_query * query_mask,
                        )
                        img_embeds, img_query = self.encode_image(
                            image_paths[1],
                            instruction_ids,
                            earlyalign=self.early_align,
                            audio_query=pre_audio_query * query_mask,
                        )
                    else:
                        audio_embeds, audio_query, _ = self.encode_audio(
                            image_paths[0],
                    ...
                    if self.early_align:
                        ...
                        if self.alignmode == 1:
                            ...
                            img_embeds, _, joint_query = self.sequence_align_v2(
                                ...
                                video_masks,
                                audio_embeds,
                                inputs["audiomasks"].to(self.device),
                                instruction_inds=instruction_ids,
                            )
                        if self.alignmode == 3:
                    ...
                    if self.cascaded == "audiogrounding":
                        ...
                        _, _, pre_video_query = self.encode_video(
                            image_paths[1], instruction_ids, earlyalign=self.early_align
                        )
                        audio_embeds, audio_query, _ = self.encode_audio(
                            image_paths[0],
                            instruction_ids,
                            earlyalign=self.early_align,
                            visual_query=pre_video_query,
                        )
                        img_embeds, img_mask, video_query = self.encode_video(
                            image_paths[1],
                            instruction_ids,
                            earlyalign=self.early_align,
                            audio_query=pre_audio_query,
                        )
                    else:
                        audio_embeds, audio_query, _ = self.encode_audio(
                            image_paths[0],
                ...
                gen_text = (
                    self.llama_tokenizer.batch_decode(
                        gen_outputs, add_special_tokens=False
                    )[0]
                    .replace("<s>", "")
                    .replace("</s>", "")
                    .strip()
            ...
            if inputs["modality"] == "image":
                ...
                if self.cascaded == "audiogrounding":
                    ...
                    if self.img_hi_rs:
                        video_embs, video_masks, video_query = self.encode_video(
                            image_paths[1],
                            instruction_ids,
                            earlyalign=self.early_align,
                            is_img=True,
                ...
                if self.diversity:
                    if self.early_align:
                        if self.ext_groupsize is None:
                            if self.low_groupsize is not None:
                                joint_query = torch.split(
                                    joint_query, joint_query.size(-1) // 2, dim=-1
                                )[1]
                                ngroups = (
                                    joint_query.size(1) // self.num_video_query_token
                                )
                                joint_query = joint_query.reshape(
                                    joint_query.size(0) * ngroups,
                                    -1,
                                    joint_query.size(-1),
                                )
                            diversity_loss = self.calc_diversity_loss(joint_query)
                        else:
                            mid_query, high_query = torch.split(
                                joint_query, joint_query.size(-1) // 3, dim=-1
                            )[1:]
                            ngroups = mid_query.size(1) // self.num_video_query_token
                            mid_query = mid_query.reshape(
                                mid_query.size(0) * ngroups, -1, mid_query.size(-1)
                            )
                            hgroups = high_query.size(1) // int(
                                self.num_video_query_token
                                * self.ext_groupsize[1]
                                / self.groupsize
                            )
                            high_query = high_query.reshape(
                                high_query.size(0) * hgroups, -1, high_query.size(-1)
                            )
                            diversity_loss = self.calc_diversity_loss(
                                mid_query
                            ) + self.calc_diversity_loss(high_query)
                    else:
                        diversity_loss = self.calc_diversity_loss(img_query)
            elif inputs["modality"] == "audiovideoimage":
                image_paths = list(zip(*image_paths))
                if self.cascaded == "audiogrounding":
                    ...
                    if self.use_npy and self.training:  # npy training
                        img_embeds, img_mask, video_query = self.load_video_npy_train(
                            image_paths[1], instructs=instructs
                        )
                    else:
                        img_embeds, img_mask, video_query = self.encode_video(
                            image_paths[1], instruction_ids, earlyalign=self.early_align
            ...
            if generate:
                ...
                if len(generate_config) != 0:
                    lora_alpha = generate_config.get(
                        "lora_alpha", self.args.get("yu_lora_alpha", 32)
                    )
                ...
                gen_text = self.llama_tokenizer.batch_decode(
                    gen_outputs, add_special_tokens=False
                )

                if len(generate_config) != 0:
                    modify_lora_layer(
                        self.llama_model, self.args.get("yu_lora_alpha", 32)
                    )
                ...
                gen_acc = (
                    ...
                    == labels.to(chosen_tokens.device).reshape(-1)
                ).to(
                    torch.long
                )  # [B*S]
    ...
    def extract_multimodal_feature(self, inputs):
        ...
        if inputs["image_paths"]:
            if (
                self.cascaded in ["audiogrounding", "bothgrounding"]
                and inputs["audio_paths"]
            ):
                _, audio_query, _ = self.encode_audio(
                    inputs["audio_paths"], instruction_ids
                )
            ...
            image_embeds, image_query = self.encode_image(
                inputs["image_paths"],
                instruction_ids,
                audio_query=audio_query,
                earlyalign=self.early_align,
        ...
        if inputs["audio_paths"]:
            ...
            if self.cascaded == "bothgrounding" and inputs["image_paths"]:
                _, pre_vis_query = self.encode_image(
                    inputs["image_paths"], instruction_ids
                )
            elif self.cascaded == "bothgrounding" and inputs["video_paths"]:
                _, _, pre_vis_query = self.encode_video(
                    inputs["video_paths"], instruction_ids
                )
            elif self.cascaded == "bothgrounding":
        ...
        if inputs["video_paths"]:
            ...
            if (
                ...
                _, pre_audio_query, _ = self.encode_audio(
                    dummy_audio_path, instruction_ids
                )
                pre_audio_query = pre_audio_query * 0
            video_embeds, video_mask, video_query = self.encode_video(
                inputs["video_paths"],
                instruction_ids,
                earlyalign=self.early_align,
                audio_query=pre_audio_query,
        ...
        if self.early_align:
            if not inputs["audio_paths"] and inputs["video_paths"]:
                audio_embeds = video_embeds.new_zeros(
                    ...
                    self.speech_encoder.config.d_model
                    if self.speech_qformer
                    else video_embeds.size(-1),
                )
                audiomasks = (
                    torch.tensor([1, 0])
                    .unsqueeze(0)
                    .repeat(video_mask.size(0), 1)
                    .to(video_embeds.device)
                )
                ...
                if inputs["image_paths"]:
                    ...
                    if inputs["audio_paths"]:
                        video_embeds = (
                            ...
                            .repeat(1, audio_embeds.size(1) // 25, 1, 1)
                            .view(image_embeds.size(0), -1, image_embeds.size(2))
                        )
                    ...
                    video_mask = audio_embeds.new_ones(video_embeds.size()[:-1]).to(
                        self.device
                    )
                    audiomasks = (
                        torch.tensor([1, 1])
                    ...
                    raise Exception(
                        "Early align mode has to have either audio or video!"
                    )
            ...
            if self.alignmode == 3:
                video_embeds = torch.cat(
                    [
                        audio_query,
                        video_query if video_query is not None else image_query,
                        video_embeds,
                    ],
                    dim=1,
                )
    ...
    def prepare_generation_embedding(self, inputs):
        ...
        sep_tokens = self.llama_tokenizer(
            textsep, add_special_tokens=False, return_tensors="pt"
        ).to(self.device)
        ...
        if self.args["use_lora"] == "true":
            ...
            sep_embeds = self.llama_model.model.model.embed_tokens(
                sep_tokens.input_ids
            ...
            sep_embeds = self.llama_model.model.embed_tokens(
                sep_tokens.input_ids
            ).expand(batch_size, -1, -1)
            # delete later
        ...
        if len(feature_embeds) == 1:
            ...
            inputs_embeds = torch.cat(
                [bos_embeds, p_before_embeds, feature_embeds[0], p_after_embeds], dim=1
    ...
    def generate_npy(self, inputs, video_name, npy_prefix):
        ...
        self.encode_video_and_save(
            video_paths, video_name, npy_prefix, instructs=instructs
        )
        return

    def encode_video_and_save(
        self,
        video_paths,
        video_name,
        npy_path_prefix,
        ...
        if self.use_blip:
            ...
            with torch.no_grad():
                ...
                npy_name = npy_name.replace(
                    "/mnt/bn/audio-visual-llm-data2/datasets/cxz/", ""
                )
                npy_name = npy_name.replace("/", "-")
