...
class Runner:
    def __init__(self, cfg, model, datasets, job_id, speaker_id: str):
        self.config = cfg
    ...
    def valid_epoch(self, epoch, split, decode=False, save_json=False):
        ...
        if save_json:
            self.save_result(
                results,
                self.output_dir,
                "eval_{}_epoch_{}_spkr_{}".format(split, epoch, self.speaker_id),
            )
    ...
    def save_result(self, result, result_dir, filename):
        ...
        if is_main_process():
            ...
            try:
                json.dump(
                    result, open(final_result_file, "w"), ensure_ascii=False, indent=4
                )
            except Exception as e:
